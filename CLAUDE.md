...
**Currently working on:** First-run guidance for MVP readiness.
**Completed this session:** Finished the user-facing empty-state guidance. Shows, songs, musical-number lists, and scene lists now explain what belongs there and direct users to the existing + button without adding duplicate empty-state buttons. Musical number, scene, and song detail sections now explain how to add their first harmony, recording, video/link, track, or PDF. Restored the dependency folder with `npm ci`; the 17 existing Jest suites pass (73 tests).
**Next steps:** Device-test the new empty states with `npx expo start -c`, especially on a fresh account. Then continue MVP readiness with the first TestFlight build and second-account data-isolation test.
**Blockers:** `npx tsc --noEmit` currently fails on an existing Expo Router typed-route error in `app/(app)/(tabs)/shows/index.tsx`: `/shows/complete` is absent from the generated route types. The empty-state change does not affect that route.
